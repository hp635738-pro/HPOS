...
(*Checking…* / *Downloading…* / *Installing…*) immediately, applies the
status the argument-free call returns even when no pushed event reaches
the window, and turns a refusal, a rejection or a check that never answers
into a visible, categorised error (`src/lib/updaterStatus.js`, executed by
