...
try {
  ...
  /* ---------------- unknown mode / service are refused ------------------- */
  {
    ...
    /* The mode table remains closed and mirrors runner.js exactly, including
       the one fixed browser-provider route. */
    assert(EXEC_MODES.length === 8 && EXEC_MODES.includes('noop')
      && EXEC_MODES.includes('browser-provider'),
    'the mode table is closed and includes the fixed browser-provider route')
  }
