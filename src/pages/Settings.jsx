 ...
 * (updater.js over electron-updater); this panel displays states and calls
 * the three argument-free actions.
 *
 * The click never relies on the pushed event stream alone. Pressing a button
 * shows the state it just started (checking / downloading / installing)
 * immediately; the status the argument-free call resolves with is applied
 * verbatim (the same payload the events carry), so up-to-date / available /
 * error render even when no event reaches this window; a refused or rejected
 * call and a check that never answers become visible errors instead of a
 * silent no-op (see lib/updaterStatus.js).
 *
 * In a plain browser window (no preload bridge) it renders the version-less
 * note instead of faking anything.
 */
function UpdatesPanel() {
  ...
  /* The click flow is async, so it reads the latest status from a ref — `u`
     from its own render would be one answer behind. */
  ...
  /**
   * Run one of the three argument-free updater actions. Whatever happens, the
   * click leaves a visible status behind: the started state right away, the
   * answer when it arrives, an error when it fails, a timeout when the check
   * never answers.
   */
  const act = (kind, fn) => {
    if (!up || acting) return
    clearWatchdog()
    setActing(true)
    applyStatus(kind === 'check' ? checkingStatus(statusRef.current) : actionStatus(kind, statusRef.current))
    if (kind === 'check') armWatchdog()
    Promise.resolve()
      .then(fn)
      .then((result) => {
        const next = statusFromResult(statusRef.current, result)
        /* The call answered while the updater still reports "checking": no
           result has been reported yet, so let the watchdog keep running
           rather than pretending the check finished. */
